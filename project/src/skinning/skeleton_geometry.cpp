...
skeleton_geometry local_to_global(skeleton_geometry const& sk_local,skeleton_parent_id const& parent_id)
{
    ...
    for (int i=0; i<N; i++)
    {
        if (i==0)
        {
            sk_global.push_back(skeleton_joint(sk_local[i].position,sk_local[i].orientation));
        }
        else
        {
            vec3 t = sk_global[parent_id[i]].position + sk_global[parent_id[i]].orientation * sk_local[i].position;
            quaternion q = sk_global[parent_id[i]].orientation * sk_local[i].orientation;

            skeleton_joint j(t,q);
...
skeleton_geometry inversed(skeleton_geometry const& skeleton)
{
    ...
    for(skeleton_joint const& joint : skeleton)
    {
        ...
        q_inv = conjugated(q);
        t_inv = q_inv*(-t);
        joint_inv.orientation = q_inv;
        joint_inv.position = t_inv;
        sk_inversed.push_back(joint_inv);
...
skeleton_geometry multiply(skeleton_geometry const& skeleton_1,skeleton_geometry const& skeleton_2)
{
    ...
    for(int k=0 ; k<N_joint ; ++k)
    {
        ...
        quaternion q_mult = q1*q2;
        vec3 t_mult = q1*t2 + t1;

        joint_mult.orientation = q_mult;
        joint_mult.position = t_mult;
        sk.push_back(joint_mult);
...
skeleton_geometry interpolated(skeleton_geometry const& skeleton_1,skeleton_geometry const& skeleton_2,float const alpha)
{
    ...
    //std::cout<<skeleton_1.size()<<std::endl;
    //std::cout<<skeleton_2.size()<<std::endl;
    ...
    for(int k=0 ; k<N_joint ; ++k)
    {
        ...
        vec3 p1 = joint_1.position;
        vec3 p2 = joint_2.position;
        quaternion q1 = joint_1.orientation;
        quaternion q2 = joint_2.orientation;

        vec3 p_new = (1-alpha)*p1 + alpha*p2;
        sk_new_joint.position = p_new;

        quaternion q_new = slerp(q1, q2, alpha);
        sk_new_joint.orientation = q_new;

        sk.push_back(sk_new_joint);
    }
