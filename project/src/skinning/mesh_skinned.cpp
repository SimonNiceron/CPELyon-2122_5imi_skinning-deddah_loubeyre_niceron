...
void mesh_skinned::apply_skinning(skeleton_geometry const& skeleton)
{
    ...
    for(int k_vertex=0 ; k_vertex<N_vertex ; ++k_vertex)
    {
        ...
        vec3 sum = vec3(0,0,0);
        vertex_weight_parameter current_weight_param = vertex_weight(k_vertex);
        for(int n_joint = 0; n_joint< current_weight_param.size(); n_joint++)
        {
            skeleton_joint current_joint;
            current_joint = skeleton[current_weight_param[n_joint].joint_id];

            quaternion q = current_joint.orientation;
            vec3 t = current_joint.position;
            skinning_weight current_weight = current_weight_param[n_joint];

            sum += current_weight.weight * (q*vertices_original_data[k_vertex] + t);
         }
        vertex(k_vertex) = sum;
    }
